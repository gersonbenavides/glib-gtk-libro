...
\begin{lstlisting}[float=p, caption={Gestión de memoria de GObjects derivados de \lstinline{GInitiallyUnowned}.}, label=oop-gobject-mem-management-floating]
...
\begin{lstlisting}[float, caption={El prototipo de la señal \lstinline{GtkButton::clicked}.}, label=oop-gobject-gtkbutton-clicked]
void
...
\begin{lstlisting}[float, caption={Cómo conectarse a una señal}, label=oop-gobject-connect-to-signal]
static void
...
\begin{lstlisting}[float, caption={Conexión a la señal de notificación para escuchar cambios en la propiedad.}, label=oop-gobject-connect-to-notify]
...
\vspace{0.7cm}
\begin{lstlisting}[caption={Vinculando dos propiedades.}, label=oop-gobject-binding-properties]
static GtkWidget *
create_check_button (MyWindow *window)
{
  GtkWidget *check_button;
